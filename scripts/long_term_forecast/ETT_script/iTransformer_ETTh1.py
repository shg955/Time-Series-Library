...
# d_model, d_ff mapping by pred_len
dim_map = {
    96: 128,
    192: 128,
    336: 512,
...
for features, target in feature_settings:
    for pred_len in pred_lens:
        d_model = dim_map[pred_len]
        d_ff = dim_map[pred_len]

        suffix = f"{features}_{target}" if not (features == "M" and target == "OT") else "M"
        base_model_id = f"iTransformer_ETTh1_96_{pred_len}_{suffix}_h"
        ...
        for extra_opts in extra_option_sets:
            ...
            # cmd_eval = ["python", "-u", "run.py"] + common_args + ["--is_training", "0"] + base_args + extra_opts


            log_file_name = f"{features}_h.txt"
            ...
            with open(log_path, "a") as f:
                ...
                # f.write(f"\n\n=== INFERENCE (is_training=0): {model_id} @ {now} ===\n")
                # subprocess.run(cmd_eval, stdout=f, stderr=subprocess.STDOUT)
