...
from layers.DeformableTST_Enc import LayerNormProxy
...
class DAttentionBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.no_off:
            ...
            pos_2[... , 1] = 0
            x_reshaped = x.reshape(B * self.n_groups, self.n_group_channels, L, 1).contiguous()

            with torch.backends.cudnn.flags(enabled=False):
                x_sampled = F.grid_sample(
                    input=x_reshaped,
                    grid=pos_2.contiguous(),
        ...
        if self.use_pe and (not self.no_off):
            ...
            if self.dwc_pe:
                ...
                rpe_rearranged = einops.rearrange(rpe_bias, 'b (g c) h w -> (b g) c h w', c=c, g=g)
                with torch.backends.cudnn.flags(enabled=False):
                    attn_bias = F.grid_sample(
                        input=rpe_rearranged.contiguous().float(),
                        grid=displacement_2.contiguous().float(),
                        mode='bilinear', align_corners=True)
